...
export function Section({ block, suppressSceneMotion = false }: SectionProps) {
  ...
  const cards = items.map((item, index) => <Card key={item.id ?? `${item.title ?? "item"}-${index}`} item={item} frame={block.itemAppearance?.frame} effect={block.itemAppearance?.effect} />);
  ...
  else if (layout === "content-switcher") body = <>{header ? <TextBlock content={header} className="section__header" /> : null}{items.length ? <ContentSwitcher items={items.map((item, index) => ({ id: item.id ?? `item-${index}`, label: item.title ?? (Array.isArray(item.eyebrow) ? item.eyebrow[0] : item.eyebrow) ?? `Option ${index + 1}`, content: <Card item={item} frame={block.itemAppearance?.frame} effect={block.itemAppearance?.effect} /> }))} /> : null}</>;
  else if (layout === "timeline") body = <>{header ? <TextBlock content={header} className="section__header" /> : null}<div className="section__timeline" data-orientation={block.timelineOrientation ?? "vertical"}>{items.length ? <Timeline items={items} /> : null}</div></>;
