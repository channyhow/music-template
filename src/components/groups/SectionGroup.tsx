...
function Panel({ id, behavior, size, align, surface, color, blocks }: { id: string; behavior: PanelBehavior; size: PanelSize; align: PanelAlign; surface: PanelSurface; color: SectionSurface; blocks: BlockRef[] }) { return <div className="sectionGroup__panel" data-panel-id={id} data-panel-behavior={behavior} data-panel-size={size} data-panel-align={align} data-panel-surface={surface} data-panel-color={color}>{renderBlocks(blocks)}</div>; }
export function SectionGroup({ group }: { group: SectionGroupData }) {
  ...
  return <div className="sectionGroup" data-layout={layout} data-panel-mode={isPanel ? mode : undefined} data-motion={reduceMotion ? "none" : group.motion?.level ?? "none"} data-preset={reduceMotion ? undefined : group.motion?.preset}>{isPanel && group.panels?.length ? group.panels.map((panel) => <Panel key={panel.id} id={panel.id} behavior={panel.behavior ?? "moving"} size={panel.size ?? defaultSize} align={panel.align ?? defaultAlign} surface={panel.surface ?? defaultSurface} color={panel.color ?? defaultColor} blocks={panel.blocks} />) : isPanel && mode === "stack" ? blocks.map((block, index) => <Panel key={block.ref} id={`${group.id}-${index + 1}`} behavior="stack" size={defaultSize} align={defaultAlign} surface={defaultSurface} color={defaultColor} blocks={[block]} />) : isPanel && mode === "scene" && blocks.length ? <><Panel id={`${group.id}-scene`} behavior="fixed" size="full" align="center" surface="transparent" color="transparent" blocks={[blocks[0]]} />{blocks.slice(1).map((block, index) => <Panel key={block.ref} id={`${group.id}-panel-${index + 1}`} behavior="moving" size={defaultSize} align={defaultAlign} surface={defaultSurface} color={defaultColor} blocks={[block]} />)}</> : renderBlocks(flowBlocks)}</div>;
}
